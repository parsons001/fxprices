...
// Both public quote endpoints only read saved snapshots.
export function createSavedQuotesHandler(store) {
  return async function GET(request) {
    ...
    const amount = query.has("gbpAmount")
      ? Number(query.get("gbpAmount"))
      : null;
    if (
      (currency &&
    ...
    try {
      const snapshots = (await store.latest({ senderCountry })).filter(
        (row) =>

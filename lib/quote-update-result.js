...
export function serializeUpdateResponse(quotes, failures) {
  ...
  for (const failure of failures) {
    ...
    if (Buffer.byteLength(JSON.stringify(body), "utf8") > MAX_UPDATE_RESPONSE_BYTES) {
      body.failures.pop();
      body.omittedFailureCount++;
      break;
    }
...
export function summarizeQuoteUpdate(context, result, httpStatus = 200) {
  ...
  const fail = (stage, reason, provider) => {
    ...
    const savedCount = result.storage?.saved === true ? (result.quotes ?? []).length : 0;
    const outcome = stage === "update"
    ...
    const action = stage === "provider" ? `${provider ?? "Provider"} could not return usable ${operation}`
      : stage === "storage" ? `Could not save ${operation} to MongoDB`
      : `Could not complete the update of ${operation}`;
    failures.push({
      ...context, stage, ...(provider ? { provider } : {}),
      message: `${action}. ${describeReason(reason)} ${outcome}`.slice(0, 1000),
    });
  };
  for (const error of result.errors ?? []) fail("provider", error.message, error.provider);
  if (result.storage?.saved === true) {
    for (const quote of result.quotes ?? []) {
      if (Object.hasOwn(quotes, quote.provider)) quotes[quote.provider]++;
    }
  } else {
    fail(result.error || httpStatus >= 400 ? "update" : "storage",
      result.storage?.error ?? result.error ?? `Quotes were not saved (HTTP ${httpStatus}).`);
  }
  return { quotes, failures };
}

function describeReason(reason) {
  const detail = String(reason || "No error details were provided").slice(0, 500);

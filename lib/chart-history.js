...
export const rangeLabel = (days) => days === 1 ? "Last 24 hours" : `Last ${days} days`;
...
export function aggregateHistory(points, days) {
  ...
  const valid = points.filter((point) => Number.isFinite(point.markup) && Number.isFinite(Date.parse(point.fetchedAt)));
  const labels = [...new Set(valid.map((point) => point.label))];
  const series = labels.map((label, index) => ({ key: `provider${index}`, label }));
  const buckets = new Map();
  for (const point of valid) {
    const timestamp = Math.floor(Date.parse(point.fetchedAt) / interval) * interval;
  ...
  const chartData = [];
  for (let timestamp = times[0]; timestamp <= times.at(-1); timestamp += interval) {

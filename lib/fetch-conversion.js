...
export async function fetchAndStoreConversion(request) {
  ...
  const amount = Number(query.get("gbpAmount") ?? 100);
  const currency = String(query.get("currency") ?? "EUR").toUpperCase();
  if (!AMOUNTS_GBP.includes(amount) || !CURRENCIES.includes(currency)) {
  ...
  const revParams = new URLSearchParams({
    amount: String(majorToMinor(amount, "GBP")),
    country: "GB",
    fromCurrency: "GBP",
    ...
    return Response.json(
      {
        ...
        storage: {
          saved: false,
          error: "Quotes fetched but could not be saved to MongoDB.",
        },
      },
      { headers: { "Cache-Control": "no-store" } },
    );

...
import { storageErrorMessage } from "../lib/storage-error.js";
...
test("storage diagnostics classify errors without exposing private driver details", () => {
  for (const [error, pattern] of [
    [{ code: 18 }, /authentication/], [{ code: 13 }, /permission/],
    [{ code: 11000 }, /duplicate/], [{ code: 121 }, /validation/],
    [{ name: "MongoOperationTimeoutError" }, /timed out/],
    [{ name: "MongoNetworkError" }, /network/],
    [{}, /could not be classified/],
  ]) {
    const message = storageErrorMessage({ ...error, message: "private credentials" });
...
test("write queue limits concurrency and continues after failed writes", async () => {
  ...
  let active = 0, peak = 0;
  const results = await Promise.allSettled(Array.from({ length: 30 }, (_, i) => run(async () => {
    active++;

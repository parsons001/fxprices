...
import { summarizeQuoteUpdate } from "../lib/quotes/quote-update-result.js";
const context = {
  mode: "send",
  senderCountry: "GB",
  sourceCurrency: "GBP",
...
test("update summary counts saved provider quotes without returning quote data", () => {
  assert.deepEqual(
    summarizeQuoteUpdate(context, { quotes, storage: { saved: true } }),
    {
      quotes: { Wise: 1, Revolut: 2 },
      failures: [],
    },
  );
});
test("partial provider failures retain the affected market and reason", () => {
  const result = summarizeQuoteUpdate(context, {
    quotes: quotes.slice(1),
    storage: { saved: true },
    errors: [{ provider: "Wise", message: "Quote unavailable (HTTP 503)" }],
  });
...
test("storage failures do not count unsaved quotes", () => {
  const result = summarizeQuoteUpdate(context, {
    quotes,
    storage: { saved: false, error: "Could not save to MongoDB" },
  });
...
test("response size stays bounded for large batches, Unicode and JSON escaping", async () => {
  const { serializeUpdateResponse, MAX_UPDATE_RESPONSE_BYTES } =
    await import("../lib/quotes/quote-update-result.js");
  for (const message of [
    "x".repeat(1000),
    "😀".repeat(1000),
    '\u0000\n"'.repeat(1000),
  ]) {
    const failures = Array.from({ length: 10000 }, () => ({
      ...context,
      message,
    }));
    const serialized = serializeUpdateResponse(
      { Wise: 0, Revolut: 0 },
      failures,
    );
    assert.ok(
      Buffer.byteLength(serialized, "utf8") <= MAX_UPDATE_RESPONSE_BYTES,
    );
    const body = JSON.parse(serialized);
    assert.equal(body.success, false);
    assert.equal(body.failureCount, failures.length);
    assert.equal(
      body.failures.length + body.omittedFailureCount,
      failures.length,
    );
...
test("small responses retain all details and success stays minimal", async () => {
  const { serializeUpdateResponse } =
    await import("../lib/quotes/quote-update-result.js");
  const counts = { Wise: 420, Revolut: 2100 };
  assert.deepEqual(JSON.parse(serializeUpdateResponse(counts, [])), {
    success: true,
    quotes: counts,
  });

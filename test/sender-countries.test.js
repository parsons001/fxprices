...
import { conversionStore, conversionDocument, countryMatch } from '../lib/conversion-store.js';
import { sendStore } from '../lib/send-store.js';
import { targetCurrencies } from '../lib/sender-countries.js';
...
test('Ireland requests EUR quotes with correct account country and EUR paid-plan estimates',async(t)=>{
 ...
 if(u.hostname==='wise.com'){
  assert.equal(u.searchParams.get('sourceCurrency'),'EUR');assert.equal(u.searchParams.get('profileCountry'),country);
  return Response.json(['BALANCE','BANK_TRANSFER'].map(payOutMethod=>({payInMethod:'BALANCE',payOutMethod,sourceCcy:'EUR',targetCcy:'GBP',midRate:0.85,total:1,sourceAmount:100,targetAmount:84.15})));
 }
 if(u.pathname.includes('remittance')){
  assert.equal(u.searchParams.get('senderCountry'),country);assert.equal(u.searchParams.get('senderCurrency'),'EUR');assert.equal(u.searchParams.get('recipientCountry'),'GB');
  return Response.json({rate:{from:'EUR',to:'GBP',rate:0.85},routes:[{id:'BANK',plans:[{id:'STANDARD',name:'Standard',fees:{currency:'EUR',total:100}}]}]});
 }
 assert.equal(u.searchParams.get('country'),country);assert.equal(u.searchParams.get('fromCurrency'),'EUR');
 return Response.json({rate:{rate:0.85},recipient:{currency:'GBP',amount:8500},plans:[{id:'STANDARD',name:'Standard',fees:{total:{currency:'EUR',amount:100},cost:{currency:'EUR',amount:10100}}}]});
 });
 for(const update of [fetchAndStoreConversion,fetchAndStoreSend]){
 const result=await(await update(new Request(`http://localhost/?senderCountry=${country}&currency=GBP&amount=100`))).json();
